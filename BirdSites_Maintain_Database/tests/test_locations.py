...
def test_locations_table_exist():
    ...
    conn.close()
    assert table_exists is not None, "Table 'locations' does not exist."

def test_locations_data_fetched():
    conn = sqlite3.connect(
        r'C:\Users\freib\Desktop\PENN_STATE\SWENG_894_Capstone\BirdSites_Database\BirdSites.db')
    df = pd.read_sql("SELECT * FROM locations", conn)
    conn.close()
    assert not df.empty, "No data found in 'locations' table."